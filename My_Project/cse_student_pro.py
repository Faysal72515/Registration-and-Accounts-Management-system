...
Dept: Computer Science & Engineering (City University)
'''
from tkinter import *
...
def login_sucess():
   ...
   #=======================================================Manage_Frame======================================
    ...
    Morebtn = Button(login_success_screen, text="More", bg="hotpink", fg="white", width=10, command=more_attendance)
...
def iExit():
    iExit = messagebox.askyesno(title="previous", message="Do you want to go previous page ")
    if iExit > 0:
...
def more_attendance():
    ...
    name_label = Label(more_screen, text="Name:", font=font_size, fg="blue")
    name_label.place(x=325, y=70)
    time_label = Label(more_screen, text="Time:", font=font_size, fg="blue")
    time_label.place(x=325, y=120)
    date_label = Label(more_screen, text="Date:", font=font_size, fg="blue")
    date_label.place(x=325, y=170)

    captured_button = Button(more_screen, text="Captured", font=("arial", 15, "bold"), bg="lime", activebackground="red",
                             width=33, command=press_captured)
    captured_button.place(x=320, y=20)
    add_button = Button(more_screen, text="add", font=("arial", 12, "bold"), bg="greenyellow", activebackground="blue",
                        width=10, command=add_students3)
    add_button.place(x=600, y=70)
    clear_button = Button(more_screen, text="clear", font=("arial", 12, "bold"), bg="red", width=10, command=clear_press)
    clear_button.place(x=600, y=120)

    exit_button = Button(more_screen, text="Exit", font=("arial", 12, "bold"), fg="white", bg="darkred", width=10,
                         command=iExit3)
    exit_button.place(x=600, y=170)

    name_entry = Entry(more_screen, bd=4, textvariable=name3_var, width=15, bg="pink", font=font_size)
    name_entry.place(x=410, y=70)
    time_entry = Entry(more_screen, bd=4, width=15, textvariable=time_var, bg="pink", font=font_size)
    time_entry.place(x=410, y=120)
    date_entry = Entry(more_screen, bd=4, width=15, textvariable=date_var, bg="pink", font=font_size)
    date_entry.place(x=410, y=170)

    Table_frame = Frame(more_screen, bd=4, relief=RIDGE, bg="gray")
    Table_frame.place(x=250, y=250, width=600, height=350)

    scroll_x = Scrollbar(Table_frame, orient=HORIZONTAL)
    scroll_y = Scrollbar(Table_frame, orient=VERTICAL)
    global Student_table
    Student_table = ttk.Treeview(Table_frame, columns=("Name", "Time", "Date"), xscrollcommand=scroll_x.set,
    ...
    Student_table['show'] = 'headings'
    Student_table.column('Name', width=150)
    Student_table.column('Time', width=150)
    Student_table.column('Date', width=150)
    Student_table.pack(fill=BOTH, expand=1)
    Student_table.bind("<ButtonRelease-1>", get_cursor3)
    fetch_data3()
...
def accunt():                      #  Third widget (for Accountbtn )
    ...
    due_account_label = Label(Detail_accunt_Frame, bg="crimson", fg="white", text="Due", font=("arial", 10, "bold"))
...
def add_account_students():
    if  total_tuition_var.get() == "" or paid_var.get()=="" or installment_var.get() == "" or due_var.get()=="":
        messagebox.showerror(title="Error", message="All fields are required")
    else:
...
#======================================================================================================================
def exit_main():
    exit_main = messagebox.askyesno(title="Quit", message="Do you want to Quit ")
    if exit_main > 0:
        root.destroy()
        return
...
def Student():        # code start and 1st widget
    ...
    # =============================================Login Frame=======================================================
    global login_frame
    login_frame = LabelFrame(root, bd=2, relief=GROOVE, bg='greenyellow')
    login_frame.place(x=300, y=180, width=800, height=450)

    global username_l_entry
    global password_l_entry

    my_name_label=Label(login_frame,text="Creator: Faysal Sarder",fg="maroon",bg="lawngreen")
    ...
    username_l_label = Label(login_frame, text="Username",fg='darkred', bg="greenyellow")
    username_l_label.place(x=230, y=180)

    username_l_entry = Entry(login_frame)
    username_l_entry.place(x=320, y=180)

    password_l_label = Label(login_frame, text="Password",fg='darkred', bg="greenyellow")
